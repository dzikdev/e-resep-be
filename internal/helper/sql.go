...
// BulkInsert is a helper function to prepare a SQL query for a bulk insert.
// BulkInsert is used over getBulkInsertSQL when all of the values are plain question
// marks (e.g. a 1-for-1 value insertion).
// The example given for getBulkInsertSQL is such a query.
func BulkInsert(SQLString string, numArgsPerRow int, numRows int) string {
